...
import {
  ...
  Wallet, Flame, Target, Shield, Cpu, HeartPulse, X, Menu,
} from "lucide-react";
  ...
  return (
    <>
      <nav className="flex flex-col gap-0.5">
        <div className="mb-1.5 px-3 text-[9px] uppercase tracking-[0.4em] text-slate-600">OS</div>
          ...
          return (
            <Link key={href} href={href} onClick={onNavigate}
              className={cn(
                ...
                active ? "bg-cyan-glow/[0.07] text-cyan-soft" : "text-slate-500 hover:bg-white/[0.04] hover:text-slate-200"
              )}
            >
              {active && <span className="absolute left-0 top-1/2 h-4 w-[2px] -translate-y-1/2 rounded-r-full bg-cyan-glow" />}
      ...
      <nav className="mt-6 flex flex-col gap-0.5">
        <div className="mb-1.5 px-3 text-[9px] uppercase tracking-[0.4em] text-slate-600">Contexto</div>
          ...
          return (
            <Link key={href} href={href} onClick={onNavigate}
              className={cn(
                ...
                active ? "bg-white/[0.05] text-slate-200" : "text-slate-600 hover:bg-white/[0.03] hover:text-slate-400"
              )}
            >
  ...
  return (
    <div className="mb-8 px-3">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <div className="text-[11px] font-semibold uppercase tracking-[0.35em] text-cyan-soft/80">Aaron OS</div>
          <div className="text-[9px] uppercase tracking-widest text-slate-600">v2 · Mission Control</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <button
        ...
        className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/[0.06] text-slate-500 transition-all hover:text-slate-300 md:hidden"
      ...
      <div
        className={cn(
          "fixed left-0 top-0 z-50 flex h-full w-64 flex-col border-r border-white/[0.05] bg-[#070b12] px-3 py-6 transition-transform duration-300 md:hidden",
